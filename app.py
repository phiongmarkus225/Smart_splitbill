...
menu = st.sidebar.radio("Go to", ["🧾 Split Bill", "📖 How to Use", "ℹ️ About"])

st.sidebar.markdown("---")
st.sidebar.caption("v1.0.0 · SmartSplit")

if menu == "📖 How to Use":
    st.markdown('<p class="header-accent">📖 How to Use SmartSplit</p>', unsafe_allow_html=True)
    st.divider()
    st.markdown(
        """
        ### Step-by-Step Guide
        1. **Upload Receipt Image:** Drag and drop or select a clear photo of your receipt (JPG/PNG).
        2. **Extract Items:** Choose between **Gemini AI** (for best accuracy) or **Offline OCR** (if you don't have an internet connection).
        3. **Review & Edit:** The app will digitize your receipt. You can easily correct any typos or wrong numbers right in the editable table!
        4. **Who is splitting?:** Enter the names of everyone involved, separated by commas (e.g. *Alice, Bob, Charlie*).
        5. **Assign items:** A grid will appear. Check the boxes to assign who ate what. If multiple people share one item, it will split that item's price evenly among them.
        6. **Check the Result:** View the live updated per-person totals. Taxes and service charges are divided proportionally based on each person's subtotal.
        7. **Export:** Click the download button to save the final split as a CSV file to share with your friends.
        """
    )
    st.info("💡 **Tip:** Make sure the grand total matches the receipt for accurate splits!")
    st.stop()

if menu == "ℹ️ About":
    st.markdown('<p class="header-accent">ℹ️ About SmartSplit AI</p>', unsafe_allow_html=True)
    st.divider()
    st.info(
        "**SmartSplit Bill AI** is an intelligent, user-friendly application designed to take the hassle out of splitting restaurant bills among friends."
    )
    st.markdown(
        """
        ### Key Features
        - 🤖 **AI-Powered Extraction:** Uses Gemini 2.5 Flash to accurately parse receipt images.
        - 📷 **Offline OCR Fallback:** Uses EasyOCR for when you don't have an API key.
        - 📝 **Fully Editable Table:** Fix any mistakes easily.
        - 👥 **Smart Item Assignment:** Intuitive checkbox grid to assign multiple people to a single item.
        - 🧮 **Fair Tax & Service Splitting:** Distributes extra charges proportionally.
        
        *Built for convenience and fairness.*
        """
    )
    st.stop()
...
if uploaded_file is not None:
    ...
    if st.session_state["extraction_error"] and st.session_state["bill_data"] is None:
        ...
        if participants and bill.get("items"):
            ...
            # 2. Text / WhatsApp Summary Export
            chat_text = f"🧾 *Bill Split Summary - {bill.get('merchant', 'Restaurant')}*\n"
            chat_text += f"💰 Grand Total: {format_currency(bill.get('total', 0), currency)}\n\n"
            for person in participants:
                info = split_result[person]
                if info["total"] > 0:
                    chat_text += f"👤 *{person}*: {format_currency(info['total'], currency)}\n"
                    for item_name, share in info["items_detail"]:
                        chat_text += f"   - {item_name} ({format_currency(share, currency)})\n"
                    chat_text += "\n"
            chat_text += "✨ _Generated by SmartSplit Bill AI_"

            with col_txt:
                st.download_button(
                    label="💬 Download as Text (WhatsApp)",
                    data=chat_text,
                    file_name="smartsplit_summary.txt",
                    mime="text/plain",
                    key="txt_export",
                    use_container_width=True,
                )

            with st.expander("📋 Copy Text for Group Chat"):
                st.code(chat_text, language="markdown")
                st.caption("Hover over the text block above and click the copy icon on the top right to share to WhatsApp / Line!")
